...
from cobra.test.test_summary import captured_output, check_in_line, check_line
...
@pytest.mark.parametrize("met, names", [("q8_c", False), ("q8_c", True)])
...
def test_metabolite_summary_to_table_with_fva(model, opt_solver, fraction, met):
...
@pytest.mark.parametrize("fraction, met", [(0.99, "fdp_c")])
def test_metabolite_summary_to_frame_with_fva(model, opt_solver, fraction, met):
    ...
    assert np.allclose(out_df["PERCENT"].tolist(), expected_percent, 1e-6, 1e-6)
