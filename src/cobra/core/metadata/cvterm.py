...
class CVTerm:
    ...
    def check_ex_res_type(
        ex_res: Union["ExternalResources", Dict, str]
    ) -> "ExternalResources":
        if ex_res is None:
...
class CVTermList(UserList):
    ...
    2. Multiple CVTerm data can be added by using "add_cvterms([])".
    """
    ...
    def query(
        ...
        >>> model.annotation.standardized.query(lambda x: x.value > 12, 'qualifier')
        """
