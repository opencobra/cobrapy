...
class HopsySampler(HRSampler):
    ...
    def _setup_chains(self, n_chains):
        if self.mcs is None or (hasattr(self.mcs, '__len__') and len(self.mcs) != n_chains):
            self.mcs = [hopsy.MarkovChain(self.hopsy_problem, self.sampler) for i in range(n_chains)]
    ...
    def sample(self, n: int, chains: int = 1, fluxes: bool = True) -> pd.DataFrame | list[pd.DataFrame]:
        ...
        """
        self._setup_chains(chains)

        _, samples = hopsy.sample(self.mcs, self.rngs, n_samples=n, thinning=self.thinning, n_procs=self.processes, )
        ...
        for _samples in samples:
            if fluxes:
                ...
                df = pd.DataFrame(
                    _samples[:, self.fwd_idx] - _samples[:, self.rev_idx],
                    columns=names,
                )
            else:
                names = [v.name for v in self.model.variables]

                df = pd.DataFrame(_samples, columns=names)
