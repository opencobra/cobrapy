...
#
if __name == 'java':
    from org.sbml.jsbml import SBMLDocument, SpeciesReference, KineticLaw, \
        Parameter
...
def create_cobra_model_from_sbml_file(sbml_filename, old_sbml=False,
    ...
    for sbml_metabolite in sbml_species:
        ...
        tmp_metabolite.compartment = \
            compartment_re.split(sbml_metabolite.getCompartment())[-1]
        if legacy_metabolite:
        ...
        if use_hyphens:
            tmp_metabolite.id = metabolite_re.split(tmp_metabolite.id)[
                -1].replace('__', '-')
        else:
        ...
        if "CHARGE" in tmp_metabolite.notes:
            ...
            try:
                ...
                if tmp_metabolite.charge is None or tmp_metabolite.charge == note_charge:
                    ...
                    msg = msg % (
                        tmp_metabolite.id, tmp_metabolite.charge, note_charge)
                    warn(msg)
    ...
    for sbml_reaction in sbml_reactions:
        if use_hyphens:
            ...
            reaction = Reaction(
                reaction_re.split(sbml_reaction.getId())[-1].replace('__',
                                                                     '-'))
        else:
            # Just in case the SBML ids are ill-formed and use -
            reaction = Reaction(
        ...
        if 'objective_coefficient' in parameter_dict:
            objective_reactions[reaction.id] = parameter_dict[
                'objective_coefficient']
        elif 'objective coefficient' in parameter_dict:
            objective_reactions[reaction.id] = parameter_dict[
                'objective coefficient']
        else:
            objective_reactions[reaction.id] = __default_objective_coefficient
    ...
    cobra_model.objective_reactions = objective_reactions
...
def parse_legacy_sbml_notes(note_string, note_delimiter=':'):
    ...
    while start_tag in note_string and end_tag in note_string:
        ...
        the_note = note_string[(note_start + len(start_tag)):note_end].lstrip(
            ' ').rstrip(' ')
        if note_delimiter in the_note:
            note_delimiter_index = the_note.index(note_delimiter)
            note_field = the_note[:note_delimiter_index].lstrip(' ').rstrip(
                ' ').replace('_', ' ').upper()
            note_value = the_note[(note_delimiter_index + 1):].lstrip(
                ' ').rstrip(' ')
...
def get_libsbml_document(cobra_model,
    ...
    for the_reaction in cobra_model.reactions:
        ...
        if len(the_reaction._metabolites) == 1:
            the_metabolite, the_coefficient = \
                list(the_reaction._metabolites.items())[0]
