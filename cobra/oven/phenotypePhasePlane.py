...
def calculate_phenotype_phase_plane(model,
    ...
    reaction1 = model.reactions.get_by_id(data.reaction1_name)
    reaction2 = model.reactions.get_by_id(data.reaction2_name)
    metabolite1 = model.metabolites.get_by_id(data.reaction1_name[3:])
    metabolite2 = model.metabolites.get_by_id(data.reaction2_name[3:])
    ...
    for i in range(data.reaction1_npoints):
        ...
        for j in range(data.reaction2_npoints):
            ...
            if model.solution.status == "optimal":
                # TODO - figure out how to get shadow prices out
