...
def _step(sampler, x, delta, fraction=None):
    ...
    valphas = ((1.0 - bounds_tol) * prob.var_bounds - x)[:, nonzero]
...
class HRSampler(object):
    ...
    n_samples : int
        ...
        sampler instance.
    problem : a python object
        A python object whose attributes define the entire sampling problem in
        matrix form. It has the following attributes:
        - equalities: all equality constraints in the model
        - b: the right side of the equality constraints
        - zero_b: a boolean indicating whether all entries in b are zero
        - inequalities: all inequality constraints in the model
        - bounds: the lower and upper bounds for the inequality constraints
        - var_bounds: the lower and upper bounds for the variables
        - projection: A projection matrix that projects an arbitrary point
          into the nullspace of the problem.
        - fixed: array of booleans indicating whether the variable at that
          index is fixed (lower bound == upper bound)
    ...
    def __init__(self, model, thinning, seed=None):
        ...
        if any(fixed_non_zero):
            self.problem.equalities = np.vstack(
                [self.problem.equalities,
                 prob.variable_equalities[fixed_non_zero, :]])
            self.problem.b = np.hstack(
                [self.problem.b, prob.variable_b[fixed_non_zero]])
            self.problem.zero_b = False
        # Set up a projection that can cast point into the nullspace
        nulls = nullspace(self.problem.equalities)
        self.problem.projection = nulls.dot(nulls.T)
        # convert bounds to a matrix and add variable bounds as well
        self.problem.bounds = np.atleast_2d(prob.bounds).T
        self.problem.var_bounds = np.array(
            [[v.lb, v.ub] for v in model.variables]).T
        # Identify variables that are fixed to a value
        # because we can ignore those during  sampling
        self.problem.fixed = (
            np.diff(self.problem.var_bounds, axis=0).flatten() <
            2 * bounds_tol)
    ...
    def generate_fva_warmup(self):
        ...
        **solver_args
        ...
        for i in idx:
            ...
            sol = np.maximum(sol, self.problem.var_bounds[0, ])
            sol = np.minimum(sol, self.problem.var_bounds[1, ])
    ...
    def _bounds_dist(self, p):
        ...
        prob = self.problem
        lb_dist = (p - prob.var_bounds[0, ]).min()
        ub_dist = (prob.var_bounds[1, ] - p).min()
        if prob.bounds.shape[0] > 0:
...
class ARCHSampler(HRSampler):
    ...
    def __single_iteration(self):
        ...
        p = self.warmup[pi, ]
        delta = p - self.center
        if not self.problem.zero_b:
            delta = self._reproject(delta)
        self.prev = _step(self, self.prev, delta)
        if self.problem.zero_b and (self.n_samples *
...
def _sample_chain(args):
    ...
    for i in range(1, sampler.thinning * n + 1):
        ...
        delta = p - center
        if not sampler.problem.zero_b:
            delta = sampler._reproject(delta)
        prev = _step(sampler, prev, delta)
        if sampler.problem.zero_b and (n_samples *
