...
class TestCobraIO:
    ...
    def compare_models(cls, name, model1, model2):
        ...
        assert (
            solution1.status == "infeasible"
            and solution2.status == "infeasible"
    ...
    def extra_comparisons(cls, name, model1, model2):
        ...
        for attr in ("id", "name"):
            assert getattr(model1.genes[0], attr) == getattr(
                model2.genes[0], attr
            )
            assert getattr(model1.genes[10], attr) == getattr(
                model2.genes[10], attr
            )
            assert getattr(model1.genes[-1], attr) == getattr(
                model2.genes[-1], attr
            )
...
def io_trial(request, data_directory):
    ...
    if request.param.reference_file:
        with open(
            join(data_directory, request.param.reference_file), "rb"
        ) as infile:
            reference_model = load(infile)
    test_model = request.param.read_function(
        join(data_directory, request.param.test_file)
    )
    test_output_filename = join(
        gettempdir(), split(request.param.test_file)[-1]
    )
...
def test_from_sbml_string(data_directory):
    ...
    TestCobraIO.compare_models(
        name="read from string", model1=model1, model2=model2
    )
...
def test_validate(data_directory):
    ...
    sbml_path = join(data_directory, "mini_fbc2.xml")
    with open(sbml_path, "r") as f_in:
        model1, errors = validate_sbml_model(
            f_in, check_modeling_practice=True
        )
        assert model1
        assert errors
...
def test_identifiers_annotation():
    ...
    for uri in ["http://identifier.org/taxonomy/9602", "https://test.com"]:
        data = _parse_annotation_info(uri)
        assert data is None
...
def test_smbl_with_notes(data_directory, tmp_path):
    ...
    for met_id in metabolite_notes:
        ...
        for annotation_key in metabolite_annotations[met_id].keys():
            assert (
                annotation_key
                in model.metabolites.get_by_id(met_id).annotation
            )
            print(met_id)
            assert (
